...
// so that we can test only the exported API from the tested package
...
func TestAdd(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(hs.Hosts) != tc.expectLen {
				t.Errorf("Expected list length %d, got %d instead\n",
					tc.expectLen, len(hs.Hosts))
			}
			if hs.Hosts[1] != tc.host {
